...
/*   Updated: 2025/03/22 15:16:46 by jroseiro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_colr colr_add_light(t_colr c, t_colr l)
{
    ...
    // Apply the light's color influence (we can adjust this factor, maybe through the intensity?)
	color_blend = 0.7f;  // How much the light color influences the object
    
    res.r = (uint8_t)fmin(255, c.r * (1.0f - color_blend + color_blend * l.r / 255.0f));
    res.g = (uint8_t)fmin(255, c.g * (1.0f - color_blend + color_blend * l.g / 255.0f));
    res.b = (uint8_t)fmin(255, c.b * (1.0f - color_blend + color_blend * l.b / 255.0f));
